...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (buf.iter = 0, NULL);
	buf.fd = fd;
	if (!buf_init(&buf, &(buf.iter), &(buf.index)))
		return ((buf.iter) = 0, NULL);
	control = 1;
	res = NULL;
	if (!gnl_r(&buf, &(buf.index), &res, control))
		return (freedom((void **)&res), (buf.iter) = 0, NULL);
	if (buf.len == 0)
		(buf.iter) = 0;
	else if ((buf.index) == buf.len -1)
		(buf.iter) = 0;
	return (res);
}
